...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    header('Location: ../client/html/edit_component.php');
    exit();
}
...
$technical_conditions = $_POST['technical_conditions'] ?? '';
...
error_log("Техническое состояние: " . $technical_conditions);
...
if (empty($name) || empty($category) || empty($technical_conditions) || empty($date_added) || empty($location) || empty($cost)) {
    $_SESSION['message'] = 'Пожалуйста, заполните все обязательные поля';
    $_SESSION['message_type'] = 'error';
    header('Location: ../client/html/edit_component.php?name=' . urlencode($name));
    exit();
}
...
try {
    ...
    error_log("technical_conditions: " . $technical_conditions);
